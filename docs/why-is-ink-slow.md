# Why is Ink slow? (and why Razzmatazz is faster)

> Part of the [Razzmatazz docs](index.md). See also: [Ink performance plan](ink-performance-plan.md) · [Render Loop](render-loop.md) · [Rendering Modes](rendering-modes.md) · [Raw Buffer API](raw-buffer.md)

Short answer: **Yoga is not the main bottleneck**. Ink and Razzmatazz both use Yoga in broadly similar ways. The biggest performance difference shows up **after layout** in the render/output pipeline.
...
## 1) What Ink and Razzmatazz do the same
...
## 3) Razzmatazz render path (what's different)

Razzmatazz keeps React+Yoga, but swaps the backend representation and diff layer:

```text
React commit
  -> Yoga layout compute
  -> renderTreeToBuffer(...) into Uint32Array [char, attr]
  -> TypeScript front/back cell diff
  -> minimal ANSI bytes
  -> stdout write
```
...
- Diff engine: `packages/core/src/renderer.ts` (`Renderer.generateDiff`)
...
Razzmatazz (high-FPS, dense redraws)
[Yoga] -> [typed buffer fill] -> [TS cell diff] -> [write]
   low         medium              low           low
```
...
- **Yoga itself is not the main differentiator** between Ink and Razzmatazz for heavy redraw workloads.
...
- Razzmatazz's biggest advantage is the **typed-cell + TypeScript diff backend** (`Uint32Array` -> cell-level diff).
...
## 8) If you wanted to make Ink faster (without switching to Razzmatazz)
...
1. **Add a cell-buffer backend inside Ink**
   ...
   - This is the approach Razzmatazz takes and targets the largest current cost center (string-heavy post-layout work).
