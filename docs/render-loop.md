# Razzmatazz Render Loop

> Part of the [Razzmatazz docs](index.md). See also: [Troubleshooting](troubleshooting.md) · [Architecture Decisions](decisions.md)

Razzmatazz uses a **game engine style render loop** to drive terminal output. This doc explains why, how it works, and how to tune it.
...
Razzmatazz maps directly onto this model:

| Game engine  | Razzmatazz                              |
| ------------ | --------------------------------------- |
...
app.paintNow(calcLayout, renderBuf)
        ...
        └── renderer.render(buffer)                    ← TS diff + stdout write
```

`onBeforeFlush` listeners fire after React fills the buffer but before the diff engine sees it — the correct insertion point for animated graphs, overlays, or anything that paints directly into the `Uint32Array`. Register via `app.onBeforeFlush(fn)` which returns an unsubscribe function. Multiple listeners are supported.
...
The Yoga layout, buffer paint, and diff engine only run when `pendingCommit` is true. The diff engine only writes ANSI bytes for cells that actually changed, so a frame where nothing visually changed produces zero terminal writes.
...
- `packages/core/src/app.ts` — `paintNow()` method: layout + buffer paint + diff render
