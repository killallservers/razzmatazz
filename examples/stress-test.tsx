/**
 * stress-test.tsx — Razzmatazz stress test
 *
...
import { render, Box, Text, useWindowSize, useApp, useInput } from '@razzmatazz/react'
...
function StressTest() {
  ...
  // We get the buffer via the Razzmatazz render context by using a side-channel:
  // render() exposes the app, and the app exposes getBuffer().
  useEffect(() => {
    const app = (globalThis as any).__razzmatazzApp
    if (!app) return

    const unsub = app.onBeforeFlush((buffer: Uint32Array, w: number, h: number) => {
      paintGrid(buffer, w, h, HEADER_ROWS, (globalThis as any).__razzmatazzFrame ?? 0)
    })
    return unsub
  }, [])
  ...
  useEffect(() => {
    ;(globalThis as any).__razzmatazzFrame = frame
  })
...
;(globalThis as any).__razzmatazzApp = app
