...
import { RazzmatazzApp, InputParser, type MouseEvent } from '@razzmatazz/core'
import { LayoutNode } from './layout.js'

export interface RazzmatazzContextProps {
  app: RazzmatazzApp
  input: InputParser
...
export const RazzmatazzContext = createContext<RazzmatazzContextProps | null>(null)
...
export const useInput = (handler: InputHandler) => {
  const context = useContext(RazzmatazzContext)

  if (!context) {
    throw new Error('useInput must be used within a Razzmatazz App environment')
  }
...
export const usePaste = (handler: PasteHandler, options: UsePasteOptions = {}) => {
  const context = useContext(RazzmatazzContext)
  if (!context) {
    throw new Error('usePaste must be used within a Razzmatazz App environment')
  }
...
/**
 * Raw access to the RazzmatazzContext — app instance + input parser.
 * Useful for advanced integrations (e.g. DevTools, custom hooks).
 */
export const useRazzmatazzContext = () => {
  const context = useContext(RazzmatazzContext)
  if (!context) throw new Error('useRazzmatazzContext must be used within a Razzmatazz App environment')
  return context
}
...
export const useApp = () => {
  ...
  if (!context) {
    throw new Error('useApp must be used within a Razzmatazz App environment')
  }

  return {
    // Ink-compatible: const { exit } = useApp()
    exit: () => context.app.quit(),
    // razzmatazz-native: direct app access
...
export const useWindowSize = () => {
  ...
  if (!context) {
    throw new Error('useWindowSize must be used within a Razzmatazz App environment')
  }
 ...
 * Razzmatazz stub: always returns false — no screen reader support.
 ...
 * Razzmatazz stub: ratatat hides the cursor during rendering (alternate screen).
...
export const useMouse = (handler: MouseHandler) => {
  const context = useContext(RazzmatazzContext)
  if (!context) throw new Error('useMouse must be used within a Razzmatazz App environment')
