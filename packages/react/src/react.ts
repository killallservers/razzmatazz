...
import { RazzmatazzApp, InputParser, createInlineLoop, terminalSize, type InlineOptions } from '@razzmatazz/core'
import { LayoutNode } from './layout.js'
import { RazzmatazzReconciler, setOnAfterCommit } from './reconciler.js'
import { renderTreeToBuffer } from './renderer.js'
import { RazzmatazzContext, useInput } from './hooks.js'
...
export interface RenderOptions {
  /** Ignored — Razzmatazz is always concurrent */
  concurrent?: boolean
  /** Target frames per second for the render loop. Default: 60 */
  maxFps?: number
  /** Ignored — Razzmatazz always patches console */
...
export interface Instance {
  ...
  /** razzmatazz-internal: direct app access */
  app: RazzmatazzApp
  /** razzmatazz-internal: input parser */
  input: InputParser
}

export function render(element: React.ReactElement, options?: RenderOptions): Instance {
  const app = new RazzmatazzApp()
  ...
  const container = RazzmatazzReconciler.createContainer(
    rootNode,
    0, // Legacy root
    null, // hydrate
    false,
  ...
  RazzmatazzReconciler.updateContainer(wrap(element) as any, container, null, () => {})
  ...
  return {
    ...
    rerender(newElement: React.ReactElement) {
      RazzmatazzReconciler.updateContainer(wrap(newElement) as any, container, null, () => {})
    },
    ...
    // Internal access — not part of Ink's public API but useful for razzmatazz-native code
    app,
    input,
  }
}
